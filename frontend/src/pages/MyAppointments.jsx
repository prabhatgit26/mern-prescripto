...
//---------------------------------------------------------------------------------
  ...
  return (
    <div>
      ...
      <p className=" text-xl text-center text-blue-800 font-normal tracking-wider bg-gradient-to-r from-blue-50 via-white to-white inline-block px-4 py-1.5 shadow-md">MY APPOINTMENTS</p>
      <hr className='text-blue-500 font-bold mt-5' />
      <div>
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] mt-3 gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {item.isCompleted && <span className="sm:min-w-48 flex items-center justify-center gap-3 text-center py-2 px-2 border border-green-200 rounded bg-gradient-to-r from-blue-400 to-green-200 text-white hover:font-semibold cursor-pointer transition-all duration-700 hover:shadow-lg ease-in-out relative group" title='Appoitment Completed' > COMPLETED <span className='flex-shrink-0  hover:scale-150 transition-all duration-1000'><img className='w-6 h-6' src={assets.thumb} alt="" /></span> </span>}

              {!item.isCompleted && !item.cancelled && item.payment && <button className='sm:min-w-48 flex items-center justify-center gap-3 text-center py-2 px-2 border rounded text-white bg-gradient-to-r from-green-600 to-green-50 hover:text-white transition-all duration-700 hover:shadow-lg ease-in-out relative group' title='Payment Completed' >PAYMENT DONE <span className='flex-shrink-0 hover:scale-125 duration-700'><img className='w-6 h-6' src={assets.security} alt="" /></span> </button>}

              {!item.isCompleted && !item.cancelled && !item.payment && <button onClick={() => appointmentStripe(item._id)} className=" text-blue-600 bg-blue-50 flex items-center justify-center gap-4 sm:min-w-48 py-2 px-4 border rounded hover:bg-gradient-to-r from-blue-600 to-blue-50 hover:text-white transition-all duration-700 hover:shadow-lg ease-in-out relative group" title='Pay Now' >PAY ONLINE <span className="flex-shrink-0"><img className='w-6 h-6 hover:translate-x-4 duration-1000' src={assets.banking} alt="" /></span> </button>}

              {!item.isCompleted && !item.cancelled && <button onClick={() => cancelAppointment(item._id)} className=' text-orange-600 text-center flex items-center justify-center gap-4 sm:min-w-48 py-2 border rounded hover:bg-gradient-to-tr from-yellow-600 to-yellow-300 hover:text-yellow-200 transition-all duration-700 hover:shadow-lg ease-in-out relative group' title='Cancel Now'>CANCEL NOW <span className="flex-shrink-0"><img className='w-6 h-6 hover:translate-y-3 duration-1000' src={assets.cancel} alt="" /></span> </button>}

              {item.cancelled && <button className='sm:min-w-48 flex items-center justify-center gap-3 py-2 border border-red-500 rounded text-white font-semibold bg-gradient-to-r from-red-600 to-red-300 hover:text-white hover:shadow-lg transition duration-300 ease-in-out relative group' title='Cancelled'> CANCELLED <span className="flex-shrink-0"><img className='w-6 h-6 hover:scale-125 duration-1000' src={assets.cancelledd} alt="" /></span>  </button>}
            </div>
          </div>
        ))}
      </div>
