        ...
        {appointments.map((item, index)=>(
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            ...
            <div className='flex-1 text-sm text-zinc-600'>
              ...
              <p className='text-sm mt-1'><span className='text-sm text-neutral-700 font-medium'>Date & Time :</span> {slotDataFormat(item.slotDate)} | {item.slotTime}</p>
            </div>
            <div></div>
            <div className='flex flex-col gap-2 justify-end'>
              ...
              {item.cancelled && <button className='sm:min-w-48 py-2 border border-red-500 rounded text-red-500 font-semibold hover:bg-red-50'>Appointment Cancelled </button>}
            </div>
          </div>
        ))}
      </div>
